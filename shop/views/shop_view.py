...
def index(request):
    ...
    if ctx is None:
        ctx = {
            ...
            'best_sellers':  list(Product.objects.filter(is_best_seller=True,  is_available=True, stock__gt=0).prefetch_related('images')[:8]),
            'new_arrivals':  list(Product.objects.filter(is_new_arrival=True,  is_available=True, stock__gt=0).prefetch_related('images')[:8]),
